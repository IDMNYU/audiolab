...
- [**Voltage Controlled Oscillator**](https://en.wikipedia.org/wiki/Voltage-controlled_oscillator) (abbrev. VCO) - an oscillator with a variable frequency controllable by an external voltage source.  This is a core component of almost any analog synthesizer.  The ARP 2600 has three VCOs, all with slightly different capabilities.  Analog oscillators can generate a variety of waveforms depending on their design. Tcherepnin designed only a few oscillator modules, preferring instead to leverage "patch programmability" to repurpose slope and timing generators to create additional oscillators.
